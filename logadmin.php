...
if(isset($_POST["submit"])){
	$result=$login->signinptg($_POST["username"],$_POST["password"]);
	
	if($result ==1){
		$_SESSION["Loginptg"]=true;
		$_SESSION["Id_petugas"]=$login->Iduser();
		$_SESSION["level"]=$login->Leveluser();
		
		if($_SESSION["level"]=="admin"){
			header("location:dasboardadmin.php");
		}
		elseif($_SESSION["level"]=="petugas"){
			header("Location:dasboardptg.php");
		}
	}
	elseif($result == 10){
...
<main class="form-signin">
  <form class="" action="logadmin.php" method="post" >
